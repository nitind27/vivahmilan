...
const TABLES_WITH_UPDATED_AT = new Set([
  ...
  'document', 'profileoption', 'couponcode', 'successstory', 'blogpost',
]);
...
function makeModel(table) {
  return {
    ...
    async findMany({ where = {}, include, orderBy, take, skip, select } = {}) {
      ...
      return resolveIncludesBatch(rows, table, include);
    },
    ...
    async count({ where = {} } = {}) {
      ...
      return row?.cnt !== undefined ? Number(row.cnt) : 0;
    },
  };
}
...
function buildWhere(where) {
  ...
  for (const [k, v] of Object.entries(where)) {
    if (k === 'OR') {
      ...
      // NOT: { type: 'MESSAGE_RECEIVED' } → type != 'MESSAGE_RECEIVED'
      const notConds = Object.entries(v).map(([nk, nv]) => {
        params.push(nv);
        return `\`${nk}\` != ?`;
      });
      if (notConds.length) conditions.push(`(${notConds.join(' AND ')})`);
...
async function resolveIncludesBatch(rows, table, include) {
  ...
  for (const [rel, opts] of Object.entries(include)) {
    ...
    const relWhere = typeof opts === 'object' && opts.where ? opts.where : {};
    const { sql: extraSql, params: extraParams } = buildWhere(relWhere);
    const extra = extraSql ? ` AND ${extraSql.replace(/^WHERE /, '')}` : '';
    const placeholders = ids.map(() => '?').join(', ');
    const order = typeof opts === 'object' ? buildOrder(opts.orderBy) : '';

    const allRows = await query(
      `SELECT * FROM \`${relTable}\` WHERE \`${fk}\` IN (${placeholders})${extra} ${order}`,
      [...ids, ...extraParams]
    );
    ...
    const take = typeof opts === 'object' ? opts.take : undefined;
    if (take) {
      for (const key of Object.keys(grouped)) {
        grouped[key] = grouped[key].slice(0, take);
      }
    }
...
const prisma = {
  ...
  siteConfig: makeModel('siteconfig'),
  pushSubscription: makeModel('pushsubscription'),
  couponCode: makeModel('couponcode'),
  couponcode: makeModel('couponcode'), // alias — some routes used lowercase
  successStory: makeModel('successstory'),
