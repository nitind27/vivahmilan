...
import { validateAboutMe, ABOUT_ME_MIN_WORDS } from '@/lib/aboutMeValidation.js';
...
export function buildApprovalChecklist(data, { mode = 'submit' } = {}) {
  if (!data) {
    return { eligible: false, checklist: [], errors: ['User not found.'] };
  }

  const { user, profile, photos, familyPhotos, documents, reportCount } = data;
  const missingFields = REQUIRED_PROFILE_FIELDS.filter((f) => {
    if (f === 'aboutMe') return !validateAboutMe(profile?.aboutMe, { required: true }).ok;
    return !profile?.[f];
  });
  // Use mandatory fields only — stored profileComplete % differs between web/Flutter (optional fields).
  const profileCompleteOk = missingFields.length === 0;
  ...
  const checklist = [
    {
      ...
      detail: missingFields.includes('aboutMe') && profile?.aboutMe
        ? `About Me needs at least ${ABOUT_ME_MIN_WORDS} words`
    ...
    },
    {
      ...
      detail: submittedOk
        ? 'Ready for review'
        : mode === 'admin'
        ? 'Optional — admin can approve directly from pending queue'
        : 'Tap Submit for Review in the app after completing all items',
    },
    {
      id: 'reports',
  ...
  if (mode !== 'admin' && !submittedOk) errors.push(APPROVAL_MESSAGES.submitted);
  if (!reportsOk) errors.push(APPROVAL_MESSAGES.reports);

  const blockingChecks = checklist.filter(c => mode === 'admin' ? c.id !== 'submitted' : c.id !== 'submitted');
  const eligible = blockingChecks.every(c => c.passed);

  return { eligible, checklist, errors, missingFields, reportCount };
}

export async function getApprovalChecklist(userId, options) {
  const data = await fetchUserVerificationData(userId);
  return buildApprovalChecklist(data, options);
}

/** User-facing checklist for onboarding — hides circular "submitted" item. */
export function formatUserSubmitChecklist(result) {
  const checklist = (result.checklist || []).filter(c => c.id !== 'submitted');
  const errors = (result.errors || []).filter(e => e !== APPROVAL_MESSAGES.submitted);
  return {
    eligible: result.eligible,
    canSubmit: result.eligible,
    checklist,
    errors,
...
export async function getUserSubmitChecklist(userId) {
  const result = await getApprovalChecklist(userId, { mode: 'submit' });
  return formatUserSubmitChecklist(result);
}
...
export async function validateAdminApproval(userId) {
  const result = await getApprovalChecklist(userId, { mode: 'admin' });
  if (result.eligible) {
    return { ok: true, ...result };
  }
  return {
