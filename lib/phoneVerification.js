...
export async function sendPhoneVerificationOtp(phone) {
  ...
  if (!(await isPhoneVerificationRequired())) {
    return { ok: false, error: 'Mobile SMS verification is turned off by admin. Enter your number and continue registration.' };
  }
...
/** Registration: enforce SMS OTP only when admin setting is ON */
export async function resolvePhoneForRegistration(phone, phoneVerificationToken) {
  const e164 = normalizePhoneToE164(phone);
  if (!e164) return { ok: false, error: 'Invalid phone number.' };

  const required = await isPhoneVerificationRequired();
  if (!required) {
    return { ok: true, e164, phoneVerified: false, verificationSkipped: true };
  }
