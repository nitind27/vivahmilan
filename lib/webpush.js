...
export async function sendPushToUser(userId, { title, body, icon, url = '/' }) {
  ...
  const iconUrl = icon || getPushIconUrl();
  ...
  const payload = JSON.stringify({
    ...
    icon: iconUrl,
    url,
    // Stable tag per user — browser replaces duplicate instead of stacking
    tag: `milan-${userId}-${url.includes('chat') ? 'chat' : 'notif'}`,
  });
    ...
    subs.map(async (sub) => {
      try {
        ...
        if (err.statusCode === 410 || err.statusCode === 404) {
          await execute('DELETE FROM pushsubscription WHERE id = ?', [sub.id]).catch(() => {});
          console.log('[WebPush] Removed expired subscription (', err.statusCode, ')');
        } else {
