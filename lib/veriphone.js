import { normalizePhoneToE164, isIndianMobileE164 } from '@/lib/phoneNormalize';
...
export async function verifyPhoneWithVeriphone(phone) {
  ...
  try {
    ...
    if (type && type !== 'mobile' && type !== 'unknown') {
      // Veriphone sometimes mislabels valid Indian mobiles (e.g. 79xxxxxxxx) as landline/VoIP
      if (isIndianMobileE164(data.e164 || e164)) {
        console.warn('[veriphone] IN mobile pattern accepted despite phone_type:', type, data.e164 || e164);
        return {
          ok: true,
          e164: data.e164 || e164,
          phone_valid: true,
