...
export async function hasSeenEarlyBirdPopup(userId) {
  ...
  const row = await queryOne(
    'SELECT earlyBirdPopupSeen FROM userpreference WHERE userId = ?',
    [userId]
  );
  return !!row?.earlyBirdPopupSeen;
}
...
/** Server: eligible and not yet claimed. Client uses sessionStorage to hide until next browser session. */
export async function shouldShowEarlyBirdPopup(userId) {
  if (!userId) return false;
  if (await hasSeenEarlyBirdPopup(userId)) return false;
