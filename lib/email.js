 ...
 * @param {string} email
 * @param {string} name
 * @param {{ accessGrant?: object | null, showAccessGift?: boolean }} [options]
 */
export async function sendAdminVerificationEmail(email, name, options = {}) {
  const { accessGrant = null, showAccessGift = true } = options;
  const gift = showAccessGift && accessGrant ? accessGrant : null;
  ...
  const giftBlock = gift
    ...
    <tr><td style="background:linear-gradient(135deg,rgba(200,164,92,.14),rgba(200,164,92,.04));
      ...
      <div style="font-size:11px;color:${tSub};text-transform:uppercase;
                  letter-spacing:3px;margin-bottom:8px;">${gift.badge}</div>
      <div style="font-size:32px;font-weight:900;color:${gold};margin-bottom:6px;">
        🎁 ${gift.headline}
      </div>
      <div style="font-size:13px;color:${tBody};line-height:1.65;margin-bottom:10px;">
        ${gift.description}
      </div>
      ${gift.validUntilLabel ? `<div style="font-size:12px;color:${tSub};">
        Active until <strong style="color:${tBody};">${gift.validUntilLabel}</strong>
      </div>` : ''}
    </td></tr>
    </table>`
    : '';

  const { buildApprovalEmailSubject } = await import('@/lib/userAccessMessaging');
  const subject = buildApprovalEmailSubject(name || 'Member', gift, showAccessGift);
  const preview = gift
    ? gift.type === 'early_bird'
      ? `${name}, your profile is verified — Early Bird access (${gift.headline}) is active!`
      : `${name}, your profile is verified — enjoy your ${gift.headline}!`
    : `${name}, your profile is now verified — start matching now!`;
...
/* ══════════════════════════════════════════════════════════════════════════
   PROFILE COMPLETION INVITE (admin → user uploads photos & ID)
══════════════════════════════════════════════════════════════════════════ */
export async function sendProfileCompletionInviteEmail(email, name, completionUrl) {
  const inner = `
    <div style="text-align:center;margin-bottom:32px;">
      <div style="font-size:48px;line-height:1;margin-bottom:12px;">📸</div>
      <div style="font-size:22px;font-weight:800;color:${tHead};">Complete your profile</div>
      <div style="font-size:13px;color:${tSub};margin-top:6px;">
        Upload your photo, family photos & identity document
      </div>
    </div>
    ...
    <p style="margin:0 0 20px;font-size:14px;color:${tBody};line-height:1.75;">
      Our team has started your <strong style="color:${gold};">Vivah Dwar</strong> matrimonial profile.
      Please use the secure link below to add your <strong>profile photo</strong>,
      <strong>family/lifestyle photos</strong>, and <strong>ID document</strong>, and review your details.
    </p>

    <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;">
    <tr><td style="background:${sect};border:1px solid ${border};border-radius:16px;padding:24px;">
      <p style="margin:0 0 8px;font-size:12px;color:${tSub};text-transform:uppercase;letter-spacing:2px;">Important</p>
      ${[
        'Open the link only from this email address',
        'You will be asked to confirm your email before continuing',
    ...
    <div style="text-align:center;margin-bottom:20px;">
      ${btn(completionUrl, 'Complete profile & upload →')}
    </div>
    ...
    <tr><td style="background:rgba(200,164,92,.06);border-left:3px solid ${gold};
                   border-radius:0 10px 10px 0;padding:14px 18px;">
      <p style="margin:0;font-size:12px;color:${tSub};line-height:1.65;">
        🔒 For your security, you must confirm the email <strong style="color:${tBody};">${email}</strong>
  ...
  await transporter.sendMail({
    from: FROM,
    to: email,
    subject: '📸 Complete your Vivah Dwar profile — photos & documents',
    html: wrap(`${name}, please complete your profile with photos and ID.`, inner),
    attachments: mailAttachments(),
  });
}
...
══════════════════════════════════════════════════════════════════════════ */
export async function sendAdminPasswordChangeEmail(email, name, { loginEmail, newPassword }) {
  const inner = `
    <div style="display:inline-block;background:rgba(200,164,92,.12);
                border:1px solid rgba(200,164,92,.35);border-radius:30px;
                padding:6px 16px;font-size:12px;color:${gold};
    ...
    <p style="margin:0 0 26px;font-size:14px;color:${tBody};line-height:1.75;">
      Your Vivah Dwar account password was reset by our admin team.
      Use the login details below to sign in. For your security, please change this password after logging in.
    </p>

    <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 28px;">
    <tr><td style="background:${sect};border:1px solid ${gold};border-radius:18px;padding:28px 24px;">
      <table width="100%" cellpadding="0" cellspacing="0">
        ${receiptRow('Login Email', loginEmail || email, true)}
        ${receiptRow('New Password', newPassword, true)}
        ${receiptRow('Login URL', `${APP_URL}/login`)}
      </table>
    </td></tr>
    </table>

    <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 28px;">
    <tr><td style="background:${sect};border:1px solid ${border};border-radius:16px;padding:24px;text-align:center;">
      <div style="font-size:11px;color:${tSub};text-transform:uppercase;letter-spacing:3px;margin-bottom:10px;">
        Your New Password
      </div>
      <div style="font-size:28px;font-weight:900;color:${gold};letter-spacing:2px;
                  font-family:'Courier New',monospace;line-height:1.3;word-break:break-all;">
        ${newPassword}
      </div>
    </td></tr>
    </table>
    ...
    <div style="text-align:center;margin-bottom:28px;">
      ${btn(`${APP_URL}/login`, 'Login to Vivah Dwar →')}
    </div>
    ...
    <tr><td style="background:rgba(200,164,92,.06);border-left:3px solid ${gold};
                   border-radius:0 10px 10px 0;padding:14px 18px;">
      <p style="margin:0;font-size:12px;color:${tSub};line-height:1.65;">
        🔒 <strong style="color:${tBody};">Keep this password private.</strong>
        Do not share it with anyone. If you did not request this change, contact support immediately.
      </p>
    </td></tr>
    </table>
  `;
  ...
  await transporter.sendMail({
    from: FROM,
    to: email,
    subject: '🔑 Your Vivah Dwar password has been reset',
    html: wrap('Your new Vivah Dwar login password is ready.', inner),
    attachments: mailAttachments(),
  });
}
