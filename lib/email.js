  ...
  const inner = `
    ...
    <tr><td style="background:rgba(200,164,92,.06);border-left:3px solid ${gold};
      ...
      <p style="margin:0;font-size:12px;color:${tSub};line-height:1.65;">
        ...
        <a href="mailto:support@vivahdwar.com" style="color:${gold};text-decoration:none;">support@vivahdwar.com</a>
...
══════════════════════════════════════════════════════════════════════════ */
export async function sendProfilePendingReminderEmail(email, name, { title, message, actionItems = [], link }) {
  ...
  const inner = `
    ...
    <div style="text-align:center;">
      ${btn(link || `${APP_URL}/profile-launch`, 'Complete My Profile →')}
    </div>
  `;
