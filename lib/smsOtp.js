/**
 * Send OTP SMS — MSG91 (India), Twilio, or console (testing).
 *
 * .env examples:
 *   SMS_PROVIDER=console          → OTP in server log + API devOtp (no real SMS)
 *   SMS_PROVIDER=msg91
 *   MSG91_AUTH_KEY=...
 *   MSG91_TEMPLATE_ID=...         → DLT OTP template from MSG91 dashboard
 */

/** True when OTP is logged / returned in API instead of real SMS */
export function usesConsoleSms() {
  const provider = (process.env.SMS_PROVIDER || '').toLowerCase();
  if (provider === 'console' || process.env.SMS_OTP_CONSOLE === '1') return true;

  const hasMsg91 = !!process.env.MSG91_AUTH_KEY?.trim();
  const hasTwilio = !!(
    process.env.TWILIO_ACCOUNT_SID?.trim() &&
    process.env.TWILIO_AUTH_TOKEN?.trim() &&
    process.env.TWILIO_PHONE_NUMBER?.trim()
  );
  if (hasMsg91 || hasTwilio) return false;
...
/**
 * @param {string} e164 e.g. +919123456789
 */
export async function sendPhoneOtpSms(e164, otp) {
  if (usesConsoleSms()) {
    console.log(`[SMS OTP console] ${e164} → OTP: ${otp}`);
