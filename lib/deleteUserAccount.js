...
import { archiveDeletedUser } from '@/lib/deletedUserArchive.js';

/**
 * Permanently remove a user and all related rows (Prisma cascades + feature tables).
 */
export async function permanentlyDeleteUserAccount(userId, { adminId = null, adminName = null } = {}) {
  ...
  try {
    await archiveDeletedUser(user, { adminId, adminName });
  } catch (e) {
    console.warn('[deleteUser] archive:', e.message);
  }
