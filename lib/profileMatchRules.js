...
const MESSAGES = {
  ...
  CASTE_MISMATCH:
    'This profile is only visible to members of the same caste (Hindu matching).',
  GOTRA_MISMATCH:
...
export function checkProfileViewAccess(viewer, targetUser, targetProfile) {
  ...
  if (vReligion) {
    ...
    if (vReligion.toLowerCase() === 'hindu') {
      const vCaste = viewer.caste?.trim();
      const tCaste = targetProfile?.caste?.trim();
      if (vCaste && tCaste && vCaste !== tCaste) {
        return { allowed: false, code: 'CASTE_MISMATCH', reason: MESSAGES.CASTE_MISMATCH };
      }
    }
  }
