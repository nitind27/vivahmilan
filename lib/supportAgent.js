...
/** Site copy is English-only; always use English support articles */
...
export function processSupportQuery(query, ctx = {}) {
  ...
  if (!q) {
    ...
    return {
      reply: personalize(g.answer[lang] || g.answer.en, { ...ctx, lang }, { greetingOnly: true, lang }),
      intent: 'greeting',
  ...
  if (['premium', 'payment'].includes(article.id) && ctx.hasFullAccess) {
    const helpReply = buildFullAccessPremiumReply(lang, ctx);
    return {
      reply: personalize(helpReply, ctx),
      intent: article.id,
      followUps: [
        { label: lang === 'hi' ? '💑 मैच' : '💑 Matches', text: 'matches' },
        { label: lang === 'hi' ? '💬 चैट' : '💬 Chat help', text: 'chat' },
        { label: lang === 'hi' ? '📝 प्रोफाइल' : '📝 Profile', text: 'profile' },
...
function fullAccessSupportNote(ctx, lang = 'en') {
  if (!ctx.hasFullAccess) return '';
  if (ctx.isEarlyBird) {
    return lang === 'hi'
      ? '\n\n🎁 **Early Bird — Free Full Access** active hai. Yeh chat **site ki jankari** ke liye hai — topic chunein ya **Live Agent** se baat karein.'
      : '\n\n🎁 **Early Bird — Free Full Access** is active on your account. This chat is for **help and questions** about the site — pick a topic below or tap **Connect Live Agent** to speak with our team.';
  }
  return lang === 'hi'
    ? '\n\n⭐ Aapka premium plan active hai. Site use karne mein madad chahiye? Topic chunein ya **Live Agent** se connect karein.'
    : '\n\n⭐ Your premium plan is active. Need help using the site? Pick a topic below or connect a **Live Agent**.';
}

function personalize(text, ctx, opts = {}) {
  let out = text;
  if (ctx.userName) {
    out = out.replace('Hello!', `Hello **${ctx.userName.split(' ')[0]}**!`);
    out = out.replace('नमस्ते!', `नमस्ते **${ctx.userName.split(' ')[0]}**!`);
  }
  if (opts.greetingOnly) {
    out += fullAccessSupportNote(ctx, opts.lang || ctx.lang || 'en');
  }
  return out;
}

function buildFullAccessPremiumReply(lang, ctx) {
  const name = ctx.userName ? ` **${ctx.userName.split(' ')[0]}**` : '';
  if (ctx.isEarlyBird) {
    return lang === 'hi'
      ? `🎁${name} aapke paas **Early Bird — Free Full Access** hai — premium features pehle se unlocked hain.\n\nYeh support chat **plans bechne** ke liye nahi — **site use**, matches, chat, profile, verification, kundali wagairah ke baare mein puch sakte hain.\n\n🧑‍💼 Personal madad? **agent** likhein ya **Connect Live Agent** dabayein.`
      : `🎁${name} you have **Early Bird — Free Full Access** — premium features are already unlocked on your account.\n\nThis support chat is for **help and how-to questions**, not to buy a plan. Ask about matches, interests, chat, profile, verification, kundali, settings, and more.\n\n🧑‍💼 Need personal help? Type **agent** or tap **Connect Live Agent**.`;
  }
  return lang === 'hi'
    ? `⭐${name} aapka **premium plan active** hai — features unlocked hain.\n\nYahan se site use, matches, chat, profile wagairah ke baare mein puch sakte hain. Plan upgrade ki zaroorat nahi jab tak aap khud upgrade na chahein.\n\n🧑‍💼 **Live Agent** se direct baat kar sakte hain.`
    : `⭐${name} your **premium plan is active** — features are unlocked.\n\nUse this chat for help with the site: matches, interests, chat, profile, settings, and more. You do not need to purchase again unless you want a different plan.\n\n🧑‍💼 You can still **Connect Live Agent** for personal support.`;
}

export function getGreetingText(lang = 'en', ctx = {}) {
  const g = ARTICLES.find((a) => a.id === 'greeting');
  return personalize(g.answer[lang] || g.answer.en, { ...ctx, lang }, { greetingOnly: true, lang });
}
