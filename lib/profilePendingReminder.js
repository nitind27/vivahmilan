...
export async function canSendPendingReminder(userId, { force = false } = {}) {
  ...
  if (user.adminVerified) return { eligible: false, reason: 'Profile already approved' };
...
export async function sendProfilePendingReminder(
  ...
  const check = await canSendPendingReminder(userId, { force });
  if (!check.eligible) {
  ...
  const actionItems = checklistActionItems(checklist);
