...
async function withRetry(fn) {
  ...
  const finalErrMsg = lastErr?.message || '';
  if (finalErrMsg.includes("reading 'length'")) {
    console.error("❌ DB failed after", MAX_RETRIES, "attempts. Network is unreachable (mysql2 broken pool bug).");
    throw new Error("DB Connection Failed: Network Unreachable (mysql2 broken pool bug)");
  }
  
  console.error("❌ DB failed after", MAX_RETRIES, "attempts:", finalErrMsg);
  throw lastErr;
}
