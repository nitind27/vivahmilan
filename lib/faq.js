...
export async function ensureFaqTable() {
  ...
  await executeDdl(`
    CREATE TABLE IF NOT EXISTS faqitem (
      ...
      KEY faqitem_home_idx (showOnHome, isActive),
  ...
  try {
    await executeDdl('ALTER TABLE faqitem ADD COLUMN showOnHome TINYINT(1) NOT NULL DEFAULT 1 AFTER showOnHelp');
  } catch { /* column exists */ }
  try {
    await executeDdl('ALTER TABLE faqitem ADD KEY faqitem_blog_post_idx (blogPostId, isActive, sortOrder)');
  } catch { /* index exists */ }
  try {
    await executeDdl('ALTER TABLE faqitem ADD KEY faqitem_home_idx (showOnHome, isActive)');
  } catch { /* index exists */ }
...
export function formatFaqRow(row) {
  ...
  return {
    ...
    showOnHome: row.showOnHome !== undefined ? !!row.showOnHome : true,
