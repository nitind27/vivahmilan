...
export async function POST(req) {
  ...
  await execute(
    ...
    [id, session.user.id, name.trim(), JSON.stringify(filters || {}), alertEnabled ? 1 : 0]
  );
...
export async function DELETE(req) {
  ...
  await ensureFeatureTables();
  await execute('DELETE FROM savedsearch WHERE id = ? AND userId = ?', [id, session.user.id]);
...
export async function PATCH(req) {
  ...
  await ensureFeatureTables();
  const existing = await queryOne('SELECT id FROM savedsearch WHERE id = ? AND userId = ?', [id, session.user.id]);
