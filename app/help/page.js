...
import { Heart, Search, ChevronDown, ChevronRight, MessageCircle, Shield, User, CreditCard, Settings, Bell } from 'lucide-react';

const CATEGORIES = [
  {
    icon: User, label: 'Account & Profile', color: 'bg-pink-100 dark:bg-pink-900/20 text-pink-600',
    faqs: [
  ...
  {
    icon: Bell, label: 'Notifications', color: 'bg-purple-100 dark:bg-purple-900/20 text-purple-600',
    faqs: [
  ...
  return (
    <div className="border-b border-gray-100 dark:border-gray-700 last:border-0">
      <button onClick={() => setOpen(!open)}
        ...
        <ChevronDown className={`w-4 h-4 flex-shrink-0 transition-transform ${open ? 'rotate-180 text-pink-500' : 'text-gray-400'}`} />
      </button>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            className="overflow-hidden">
            <p className="text-gray-500 dark:text-gray-400 text-sm pb-4 leading-relaxed">{a}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <div className="pt-16">
        ...
        <div className="max-w-4xl mx-auto px-4 py-12">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-12">
              {CATEGORIES.map((cat, i) => (
                <button key={i} onClick={() => setActiveCategory(activeCategory === i ? null : i)}
                  className={`p-4 rounded-2xl border-2 text-left transition-all ${activeCategory === i ? 'border-pink-400 bg-pink-50 dark:bg-pink-900/10' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-pink-300'}`}>
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 ${cat.color}`}>
                    <cat.icon className="w-5 h-5" />
                  </div>
                  <p className="font-semibold text-sm">{cat.label}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{cat.faqs.length} articles</p>
                </button>
              ))}
            </div>
          )}
          ...
          {/* Still need help */}
          <div className="mt-12 bg-gradient-to-r from-pink-500 to-purple-600 rounded-2xl p-6 text-white text-center">
            ...
            <Link href="/contact" className="inline-block bg-white text-pink-600 px-6 py-2.5 rounded-xl font-semibold text-sm hover:bg-gray-50 transition-colors">
              Contact Us
            </Link>
          </div>
        </div>
